            ...
            {isVisible && (
                <button onClick={scrollToTop} className="fixed bottom-10 right-10 w-14 h-14 bg-gray-200 cursor-pointer rounded-full flex items-center justify-center shadow-lg z-50">
                    <svg className="absolute w-14 h-14 -rotate-90" viewBox="0 0 40 40">
                        <circle cx="20" cy="20" r={r} stroke="rgba(107, 114, 128, 0.2)" strokeWidth="4" fill="none" />
                        <circle cx="20" cy="20" r={r} stroke="rgba(107, 114, 128, 1)" strokeWidth="4" fill="none" strokeDasharray={circumference} strokeDashoffset={strokeDashoffset} strokeLinecap="round" className="transition-all duration-150 ease-out" />
                    </svg>
                    <FaArrowUp className="text-gray-500 text-2xl z-10" />
                </button>
            )}
        </>
    );
