...
const ProductInfo = ({ product, selectedColor, setSelectedColor, renderStars }) => {
  ...
  const handleAddToBag = () => {
    ...
    console.log('added this: ', product.name, selectedColor, selectedSize)
  };
  ...
  return (
    <div className="w-full max-md:px-3 md:pr-10 pb-8">
        {/* having added addtobasket and wishlist, this will be deleted */}
        {showNotification && (
            <div className="fixed top-22 right-40 bg-black text-white px-4 py-2 rounded-md z-50 shadow-lg">
              {showNotification}
            </div>
        )}
        ...
        <div className="mb-4">
          ...
          {/*Add to Bag */}
          <button onClick={handleAddToBag} className="mb-6 w-full bg-black text-white py-3 px-4 rounded-full cursor-pointer hover:bg-white border-3 hover:text-black transition-colors">
              Add to bag
          </button>
